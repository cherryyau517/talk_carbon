class ActivityLogsController < ApplicationController
  ...
  def index
    ...
    @activity_logs = @q.result(distinct: true).includes(:user,
                                                        :activity_type).page(params[:page]).per(10)
  end
  ...
  def activity_log_params
    params.require(:activity_log).permit(:activity_name, :activity_type_id,
                                         :date, :time, :activity_remarks, :addition, :carbon_amount, :user_id)
  end
end
